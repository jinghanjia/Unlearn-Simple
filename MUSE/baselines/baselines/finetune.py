...
def create_sam_optimizer(model, weight_decay, lr, betas, rho):
    ...
    optimizer = SAMAdamW(optimizer_grouped_parameters, **optimizer_kwargs)
...
def smoothft(
    ...
    optimizer = create_sam_optimizer(model, 0.0, learning_rate, (0.9, 0.999), rho)
    trainer = SMOOTHFT(
        model=model,
        tokenizer=tokenizer,
        train_dataset=dataset,
